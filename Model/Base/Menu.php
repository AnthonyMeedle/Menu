...
abstract class Menu implements ActiveRecordInterface
{
    ...
    /**
     * @var        ObjectCollection|ChildMenuVersion[] Collection to store aggregation of ChildMenuVersion objects.
     */
    protected $collMenuVersions;
    protected $collMenuVersionsPartial;
     ...
     * @var ObjectCollection
     */
    protected $menuVersionsScheduledForDeletion = null;
    ...
    public function setVersion($v)
    {
        ...
        if ($this->version !== $v) {
            $this->version = $v;
            $this->modifiedColumns[MenuTableMap::VERSION] = true;
        }
    ...
    public function setVersionCreatedAt($v)
    {
        ...
        if ($this->version_created_at !== null || $dt !== null) {
            if ($dt !== $this->version_created_at) {
                ...
                $this->modifiedColumns[MenuTableMap::VERSION_CREATED_AT] = true;
            }
        } // if either are not null
    ...
    public function setVersionCreatedBy($v)
    {
        ...
        if ($this->version_created_by !== $v) {
            ...
            $this->modifiedColumns[MenuTableMap::VERSION_CREATED_BY] = true;
        }
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        try {
            ...
            if ($this->isVersioningNecessary()) {
                ...
                if (!$this->isColumnModified(MenuTableMap::VERSION_CREATED_AT)) {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->menuItemsScheduledForDeletion !== null) {
                if (!$this->menuItemsScheduledForDeletion->isEmpty()) {
                    \Menu\Model\MenuItemQuery::create()
            ...
            if ($this->menuVersionsScheduledForDeletion !== null) {
                if (!$this->menuVersionsScheduledForDeletion->isEmpty()) {
                    \Menu\Model\MenuVersionQuery::create()
                        ->filterByPrimaryKeys($this->menuVersionsScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
                    $this->menuVersionsScheduledForDeletion = null;
                }
            }

                if ($this->collMenuVersions !== null) {
            foreach ($this->collMenuVersions as $referrerFK) {
                    if (!$referrerFK->isDeleted() && ($referrerFK->isNew() || $referrerFK->isModified())) {
                        $affectedRows += $referrerFK->save($con);
                    }
                }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(MenuTableMap::VERSION)) {
            $modifiedColumns[':p' . $index++]  = 'VERSION';
        }
        if ($this->isColumnModified(MenuTableMap::VERSION_CREATED_AT)) {
            $modifiedColumns[':p' . $index++]  = 'VERSION_CREATED_AT';
        }
        if ($this->isColumnModified(MenuTableMap::VERSION_CREATED_BY)) {
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            if (null !== $this->collMenuItems) {
                $result['MenuItems'] = $this->collMenuItems->toArray(null, true, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
            if (null !== $this->collMenuVersions) {
                $result['MenuVersions'] = $this->collMenuVersions->toArray(null, true, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
        }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(MenuTableMap::VERSION_CREATED_AT)) $criteria->add(MenuTableMap::VERSION_CREATED_AT, $this->version_created_at);
        if ($this->isColumnModified(MenuTableMap::VERSION_CREATED_BY)) $criteria->add(MenuTableMap::VERSION_CREATED_BY, $this->version_created_by);
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy) {
            ...
            foreach ($this->getMenuVersions() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addMenuVersion($relObj->copy($deepCopy));
                }
            }
    ...
    public function initRelation($relationName)
    {
        ...
        if ('MenuVersion' == $relationName) {
            return $this->initMenuVersions();
        }
    }
    ...
    public function initMenuItems($overrideExisting = true)
    {
        ...
        $this->collMenuItems->setModel('\Menu\Model\MenuItem');
    }
     ...
     * @return Collection|ChildMenuItem[] List of ChildMenuItem objects
     * @throws PropelException
     */
    public function getMenuItems($criteria = null, ConnectionInterface $con = null)
    {
    ...
    /**
     * Sets a collection of MenuItem objects related by a one-to-many relationship
    ...
    public function addMenuItem(ChildMenuItem $l)
    {
        ...
        if (!in_array($l, $this->collMenuItems->getArrayCopy(), true)) { // only add it if the **same** object is not already associated
    ...
    /**
     * @param MenuItem $menuItem The menuItem object to add.
     */
    protected function doAddMenuItem($menuItem)
    {
    ...
    /**
     * @param  MenuItem $menuItem The menuItem object to remove.
     * @return ChildMenu The current object (for fluent API support)
     */
    public function removeMenuItem($menuItem)
    {
        if ($this->getMenuItems()->contains($menuItem)) {
            $this->collMenuItems->remove($this->collMenuItems->search($menuItem));
    ...
    /**
     * Clears out the collMenuVersions collection
     *
     * This does not modify the database; however, it will remove any associated objects, causing
     * them to be refetched by subsequent calls to accessor method.
     *
     ...
     * @see        addMenuVersions()
     */
    public function clearMenuVersions()
    {
        $this->collMenuVersions = null; // important to set this to NULL since that means it is uninitialized
    }

    /**
     * Reset is the collMenuVersions collection loaded partially.
     */
    public function resetPartialMenuVersions($v = true)
    {
        $this->collMenuVersionsPartial = $v;
    }

    /**
     * Initializes the collMenuVersions collection.
     *
     * By default this just sets the collMenuVersions collection to an empty array (like clearcollMenuVersions());
     * however, you may wish to override this method in your stub class to provide setting appropriate
     * to your application -- for example, setting the initial array to the values stored in database.
     *
     * @param      boolean $overrideExisting If set to true, the method call initializes
     ...
     * @return void
     */
    public function initMenuVersions($overrideExisting = true)
    {
        if (null !== $this->collMenuVersions && !$overrideExisting) {
            return;
        }
        $this->collMenuVersions = new ObjectCollection();
        $this->collMenuVersions->setModel('\Menu\Model\MenuVersion');
    }

    /**
     * Gets an array of ChildMenuVersion objects which contain a foreign key that references this object.
     *
     * If the $criteria is not null, it is used to always fetch the results from the database.
     * Otherwise the results are fetched from the database the first time, then cached.
     * Next time the same method is called without $criteria, the cached collection is returned.
     ...
     * @param      Criteria $criteria optional Criteria object to narrow the query
     * @param      ConnectionInterface $con optional connection object
     * @return Collection|ChildMenuVersion[] List of ChildMenuVersion objects
     * @throws PropelException
     */
    public function getMenuVersions($criteria = null, ConnectionInterface $con = null)
    {
        $partial = $this->collMenuVersionsPartial && !$this->isNew();
        if (null === $this->collMenuVersions || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collMenuVersions) {
                // return empty collection
                $this->initMenuVersions();
            } else {
                $collMenuVersions = ChildMenuVersionQuery::create(null, $criteria)
                    ->filterByMenu($this)
                    ->find($con);

                if (null !== $criteria) {
                    if (false !== $this->collMenuVersionsPartial && count($collMenuVersions)) {
                        $this->initMenuVersions(false);

                        foreach ($collMenuVersions as $obj) {
                            if (false == $this->collMenuVersions->contains($obj)) {
                                $this->collMenuVersions->append($obj);
                            }
                        }

                        $this->collMenuVersionsPartial = true;
                    }

                    reset($collMenuVersions);

                    return $collMenuVersions;
                }

                if ($partial && $this->collMenuVersions) {
                    foreach ($this->collMenuVersions as $obj) {
                        if ($obj->isNew()) {
                            $collMenuVersions[] = $obj;
                        }
                    }
                }

                $this->collMenuVersions = $collMenuVersions;
                $this->collMenuVersionsPartial = false;
            }
        }

        return $this->collMenuVersions;
    }

    /**
     * Sets a collection of MenuVersion objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param      Collection $menuVersions A Propel collection.
     * @param      ConnectionInterface $con Optional connection object
     * @return   ChildMenu The current object (for fluent API support)
     */
    public function setMenuVersions(Collection $menuVersions, ConnectionInterface $con = null)
    {
        $menuVersionsToDelete = $this->getMenuVersions(new Criteria(), $con)->diff($menuVersions);


        //since at least one column in the foreign key is at the same time a PK
        //we can not just set a PK to NULL in the lines below. We have to store
        //a backup of all values, so we are able to manipulate these items based on the onDelete value later.
        $this->menuVersionsScheduledForDeletion = clone $menuVersionsToDelete;

        foreach ($menuVersionsToDelete as $menuVersionRemoved) {
            $menuVersionRemoved->setMenu(null);
        }

        $this->collMenuVersions = null;
        foreach ($menuVersions as $menuVersion) {
            $this->addMenuVersion($menuVersion);
        }

        $this->collMenuVersions = $menuVersions;
        $this->collMenuVersionsPartial = false;
    ...
    /**
     * Returns the number of related MenuVersion objects.
     *
     * @param      Criteria $criteria
     * @param      boolean $distinct
     * @param      ConnectionInterface $con
     * @return int             Count of related MenuVersion objects.
     * @throws PropelException
     */
    public function countMenuVersions(Criteria $criteria = null, $distinct = false, ConnectionInterface $con = null)
    {
        $partial = $this->collMenuVersionsPartial && !$this->isNew();
        if (null === $this->collMenuVersions || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collMenuVersions) {
            ...
            if ($partial && !$criteria) {
                return count($this->getMenuVersions());
            }

            $query = ChildMenuVersionQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
        ...
        return count($this->collMenuVersions);
    }

    /**
     * Method called to associate a ChildMenuVersion object to this object
     * through the ChildMenuVersion foreign key attribute.
     *
     * @param    ChildMenuVersion $l ChildMenuVersion
     * @return   \Menu\Model\Menu The current object (for fluent API support)
     */
    public function addMenuVersion(ChildMenuVersion $l)
    {
        if ($this->collMenuVersions === null) {
            $this->initMenuVersions();
            $this->collMenuVersionsPartial = true;
        ...
        if (!in_array($l, $this->collMenuVersions->getArrayCopy(), true)) { // only add it if the **same** object is not already associated
            $this->doAddMenuVersion($l);
        }
    ...
    /**
     * @param MenuVersion $menuVersion The menuVersion object to add.
     */
    protected function doAddMenuVersion($menuVersion)
    {
        $this->collMenuVersions[]= $menuVersion;
        $menuVersion->setMenu($this);
    }

    /**
     * @param  MenuVersion $menuVersion The menuVersion object to remove.
     * @return ChildMenu The current object (for fluent API support)
     */
    public function removeMenuVersion($menuVersion)
    {
        if ($this->getMenuVersions()->contains($menuVersion)) {
            $this->collMenuVersions->remove($this->collMenuVersions->search($menuVersion));
            if (null === $this->menuVersionsScheduledForDeletion) {
                $this->menuVersionsScheduledForDeletion = clone $this->collMenuVersions;
                $this->menuVersionsScheduledForDeletion->clear();
            }
            $this->menuVersionsScheduledForDeletion[]= clone $menuVersion;
            $menuVersion->setMenu(null);
        }
    ...
    public function clearAllReferences($deep = false)
    {
        if ($deep) {
            ...
            if ($this->collMenuVersions) {
                foreach ($this->collMenuVersions as $o) {
                    $o->clearAllReferences($deep);
                }
            }
        } // if ($deep)
    ...
    public function toVersion($versionNumber, $con = null)
    {
        ...
        if (!$version) {
            throw new PropelException(sprintf('No ChildMenu object found with version %d', $version));
        }
    ...
    {
        $v = ChildMenuVersionQuery::create()
            ->filterByMenu($this)
            ->orderByVersion('desc')
            ->findOne($con);
        if (!$v) {
    ...
    {
        return ChildMenuVersionQuery::create()
            ->filterByMenu($this)
     ...
     * @param   ConnectionInterface $con the connection to use
     *
     * @return  ObjectCollection A list of ChildMenuVersion objects
     */
    public function getAllVersions($con = null)
    {
        $criteria = new Criteria();
        $criteria->addAscendingOrderByColumn(MenuVersionTableMap::VERSION);

        return $this->getMenuVersions($criteria, $con);
    }
     ...
     * @return PropelCollection|array \Menu\Model\MenuVersion[] List of \Menu\Model\MenuVersion objects
     */
    public function getLastVersions($number = 10, $criteria = null, $con = null)
    {
        $criteria = ChildMenuVersionQuery::create(null, $criteria);
        $criteria->addDescendingOrderByColumn(MenuVersionTableMap::VERSION);
